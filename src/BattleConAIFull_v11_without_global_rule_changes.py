...
# code sane way of checking first beats
...
# Active ante triggers (Luc and Heketch's moves) are reported twice when
# there's a Cancel
    ...
    def simulate(self, s0, s1, state = None):
        ...
        try:
            ...
            if state.stage <=0:
                ...
                # Pulse - retreat and skip directly to cycle and evaluation phase.
                # Not using execute_move, because Pulse negates all other effects
                # (including status effects from last beat)
                if len (pulsing_players) > 0:
                    left_player = (self.player[0]
                        if self.player[0].position < self.player[1].position
                        else self.player[1])
                    right_player = left_player.opponent
                    # double pulse - throw everyone back
                    if len (pulsing_players) == 2:
                            left_player.position = 0
                            right_player.position = 6
                    # one pulse - fork to decide push/retreat
                    else:
                        pairs = [(lpos,rpos)
                                  for lpos in range (0,left_player.position+1)
                                  for rpos in range (right_player.position,7)]
                        prompt = "Choose positions after Pulse:"
                        options = []
                        if pulsing_players[0].is_user and self.interactive_mode:
                            lp, rp = left_player.position, right_player.position
                            for p in pairs:
                                left_player.position = p[0]
                                right_player.position = p[1]
                                options.append (self.get_basic_board())
                            left_player.position, right_player.position = lp, rp
                        left_player.position, right_player.position = \
                                 pairs [self.make_fork (len(pairs),
                                                        pulsing_players[0],
                                                        prompt, options)]
                    if self.reporting:
                        self.report ('Pulse:')
...
##            card.movement_reaction (mover, old_position, direct)
    ...
    # direct: the move skips intervening spaces, and is immune to blocks
    ...
    def inner_execute_move (self, mover, moves, direct):
        ...
        # (incorporating ruling about direct movement never being blocked)
        if direct:
            possible = list (dests)
        else:
            if mover == self:
            ...
            if len (blocked) == 0:
                ...
                else: # can't really reach this, now that direct is immune to blocking
    ...
    # return possible destination, taking into account board size
...
##    def reveal_trigger (self):
##        me = self.me
##        if self.game.distance() <= me.get_maxrange() and \
##           self.game.distance() >= me.get_minrange() :
##            me.judgment_catch_in_reveal_phase = True
##            if self.game.reporting:
##                self.game.report (self.opponent.name + " is immobilized")
##    def blocks_movement (self, direct):
##        if self.me.judgment_catch_in_reveal_phase:
##            # block all movement
##            return set(xrange(7))
##        else:
##            # block nothing
...
class Haste (Paradigm):
    ...
    def blocks_movement (self, direct):
        ...
        if direct:
            return set()
        # otherwise, block switching sides
        if self.me.position > self.opponent.position:
